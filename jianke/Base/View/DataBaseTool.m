...
/** 查询发过的群发消息历史记录 */
+ (NSMutableArray *)groupMessagesWithJobId:(NSString *)jobId withIndex:(NSInteger)index
{
    NSMutableArray *messageArray = [NSMutableArray array];
    NSString *querySqlCmd = [NSString stringWithFormat:@"select post_content from t_group_message where job_id = '%@' order by post_time DESC limit %ld,%ld", jobId, (long)index, (long)kNumPerPage];
    
    FMResultSet *results = [_groupMessageDB executeQuery:querySqlCmd];
    
    while ([results next]) {
        
        NSData *data = [results dataForColumn:@"post_content"];
        GroupMessageModel *model = [NSKeyedUnarchiver unarchiveObjectWithData:data];
...
/** 缓存群发消息 */
+ (void)saveGroupMessage:(GroupMessageModel *)message
{
    NSData *data = [NSKeyedArchiver archivedDataWithRootObject:message];
    
    if (![_groupMessageDB executeUpdateWithFormat:@"insert into t_group_message (job_id, post_time, post_content) values (%@, %@, %@)", message.jobId.stringValue, message.postTime.stringValue, data]) {
