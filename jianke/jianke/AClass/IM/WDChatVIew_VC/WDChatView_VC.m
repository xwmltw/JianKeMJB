...
@interface WDChatView_VC ()<XHAudioPlayerHelperDelegate>{
...
- (void)systemTypeMediaOnclickEventWith:(WdMessage*)message andImSysteMsg:(ImSystemMsg*)sysMsg{
    ...
    switch (sysMsg.notice_type.intValue) {
        ...
        {
            // 获取岗位模型
            [[UserData sharedInstance] getJobDetailWithJobId:sysMsg.job_id.stringValue Block:^(JobDetailModel *jobDetail) {
               
                JobModel *jobModel = jobDetail.parttime_job;
                
                ManagerType managerType;
                if (sysMsg.is_bd_account && sysMsg.is_bd_account.integerValue) { // 是BD岗位, 跳包招管理
                    managerType = ManagerTypeBD;
                } else { // 不是BD岗位, 跳兼客管理页面
                    managerType = ManagerTypeEP;
                }
                    
                JiankeCollection_VC *vc = [[JiankeCollection_VC alloc] init];
                vc.jobId = jobModel.job_id.stringValue;
                vc.managerType = managerType;
                vc.jobModel = jobModel;
                vc.isAccurateJob = @(0);
                [self.navigationController pushViewController:vc animated:YES];
            }];
            
//            ApplyJKController *vc = [[ApplyJKController alloc]init];
//            vc.jobId = [NSString stringWithFormat:@"%@",sysMsg.job_id];
//            [self.navigationController pushViewController:vc animated:YES];
//            //岗位管理界面
        }
        ...
        {
            ReportRecord_VC *vc = [[ReportRecord_VC alloc]init];
            WdMessage *wdMessage = (WdMessage *)message;
            ImSystemMsg *sysMsg = wdMessage.obj_id;
            
            vc.job_title = sysMsg.job_title;
            NSNumber *punchId = sysMsg.punch_the_clock_request_id;
            vc.punch_the_clock_request_id = punchId.description;
...
//        case WdSystemNoticeType_JkApply:                // 1 兼客申请岗位
        ...
        case WdSystemNoticeType_BDSendBillForPayToEP:       // 60: bd发送账单给雇主支付
        {
            ...
            vc.jobBillId = sysMsg.job_bill_id.stringValue;
            [self.navigationController pushViewController:vc animated:YES];
        }
            break;
