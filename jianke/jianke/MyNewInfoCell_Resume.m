...
- (void)setModel:(JKModel *)model{
    ...
    if ([[UserData sharedInstance] isLogin]) {
        ...
        if (model.complete) {
            ...
            NSMutableAttributedString *mutablStr = [[NSMutableAttributedString alloc] initWithString:str attributes:@{NSForegroundColorAttributeName: MKCOLOR_RGB(251, 93, 96), NSFontAttributeName: [UIFont systemFontOfSize:14.0f], NSBaselineOffsetAttributeName: @2}];
            [mutablStr addAttributes:@{NSForegroundColorAttributeName: MKCOLOR_RGB(251, 93, 96), NSFontAttributeName: [UIFont systemFontOfSize:18.0f], NSBaselineOffsetAttributeName: @0} range:NSRangeFromString(@"{0, 4}")];
