...
- (void)initUIWithCity{
    ...
    _jobExpressVC.tableSectionView = self.hotView;
...
- (UIView *)hotView{
    if (!_hotView) {
        _hotView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 80)];
        UIButton *btn = [[UIButton alloc]initWithFrame:_hotView.bounds];
        [btn setImage:[UIImage imageNamed:@"Home_HotBtn"] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(hotViewBtn:) forControlEvents:UIControlEventTouchUpInside];
        [_hotView addSubview:btn];
    }
    
    return _hotView;
}
- (void)hotViewBtn:(UIButton *)sender{
...
- (void)initNavigationButton{
    ...
    self.localBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
...
//    searchBtn.frame = CGRectMake(16, 8, SCREEN_WIDTH-80-16, 28);
//    [searchBtn setTitle:@"搜索你喜欢的兼职,雇主" forState:UIControlStateNormal];
//    [searchBtn.titleLabel setFont:[UIFont systemFontOfSize:13.0f]];
//    [searchBtn.titleLabel setTextColor:MKCOLOR_RGB(255, 255, 255)];
//    [searchBtn setBackgroundColor:MKCOLOR_RGBA(3, 3, 3, 0.09)];
//    [searchBtn setCornerValue:12];
...
//    self.navigationItem.leftBarButtonItems = @[nevgativeSpaceRight,rightItem];
}
...
- (void)onScrollViewDidScroll:(UIScrollView *)scrollView {
    UIColor *color = MKCOLOR_RGB(251, 93, 96);
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (self.leftTableView) {
        ...
        [self.navigationController.navigationBar lt_setBackgroundColor:[MKCOLOR_RGB(251, 93, 96) colorWithAlphaComponent:0]];
    }
}
...
- (void)initTableHeaderView{
    ...
    [image setImage:[UIImage imageNamed:@"Home_hover"]];
    [self.cycleBannerSV addSubview:image];
    [image mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@12);
        make.left.bottom.right.equalTo(self.cycleBannerSV);
    }];
